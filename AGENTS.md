...
所有提交必须使用以下类型之一作为前缀，首字母大写格式：

```
<Type>: 简短描述
```

**一次提交只允许一个主类型。** 如果同时涉及功能和数据，优先记录功能类型：

```
❌ Feat: 新增历史通道系统并增加丝绸之路数据
✅ Feat: 新增历史通道系统
```
...
| 类型 | 用途 | 示例 |
|------|------|------|
| **Feat** | 新增功能、新能力、新模块 | `Feat: 新增历史通道系统` |
| **Data** | 历史内容、城市资料、时间线数据等知识库更新 | `Data: 增加杭州时间线` |
| **Fix** | 修复 Bug 或错误行为 | `Fix: 修复时间线排序错误` |
| **Style** | 视觉与样式调整（不涉及业务逻辑） | `Style: 优化城市详情页布局` |
| **Refactor** | 代码重构（不新增功能，不修复 Bug） | `Refactor: 提取时间线组件` |
| **Docs** | 文档更新（README、AGENTS 等） | `Docs: 更新项目说明` |
| **Chore** | 工程维护（依赖升级、构建配置、CI/CD） | `Chore: 更新项目依赖` |
...
本项目是历史城市知识项目。以下内容属于**知识内容更新**，使用 `Data` 而非 `Feat`：

- 城市数据（新增城市、补充时间线、修正历史资料）
- 历史通道数据
- 地理力量数据
- 历史说明
...
只有**新增功能模块**（新组件、新系统、新交互能力）才使用 `Feat`：
